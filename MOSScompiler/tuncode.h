...
enum ExprKind { ID_EXPR, LITERAL_EXPR, TEMP_EXPR, STR_EXPR };

struct ExprRec // information about a constant, variable, string, or
               // an intermediate (temporary) result
{
   ...
   string      a;   // used when kind is STR_EXPR
};
...
*/
	...
	void AssignSTR(const ExprRec & target, const ExprRec & source);
	// Produces the assembly code for an assignment from Source to Target.
	...
	void ProcessString(ExprRec& e);
	// takes string contents from token buffer
    // to string expression object
	...
	// Produces the assembly code to write a sting
...
private:
	...
	void STREnter(const string & s);
	// Enters string into stringTable
	...
	// Works like ExtractExpr with stringTable instead of symbolTable
	...
	// Returns true if in stringTable 
